# FileUtils module from Ruby 1.8 Standard Library.
# Modified to be wrapped under the AMS module.
...
module AMS
  module FileUtils
    ...
    #
    module Verbose
      include ::AMS::FileUtils
      @fileutils_output  = $stderr
      @fileutils_label   = ''
      ::AMS::FileUtils.collect_method(:verbose).each do |name|
      ...
      extend self
      class << self
        ::AMS::FileUtils::METHODS.each do |m|
          public m
        end
      end
    end
    ...
    module NoWrite
      ...
      @fileutils_output  = $stderr
      @fileutils_label   = ''
      ::AMS::FileUtils.collect_method(:noop).each do |name|
        module_eval(<<-EOS, __FILE__, __LINE__ + 1)
          def #{name}(*args)
